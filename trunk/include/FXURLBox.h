...
*********************************************************************************
* $Id: FXComboBox.h,v 1.44 2005/04/15 02:26:42 fox Exp $                        *
********************************************************************************/
#ifndef FXURLBOX_H
#define FXURLBOX_H
...
enum {
  ...
  URLBOX_REPLACE        = 0x00020000,         // Replace current item with typed text
  URLBOX_INSERT_BEFORE  = 0x00040000,         // Typed text inserted before current
  URLBOX_INSERT_AFTER   = 0x00060000,         // Typed text inserted after current
  URLBOX_INSERT_FIRST   = 0x00080000,         // Typed text inserted at begin of list
  URLBOX_INSERT_LAST    = 0x00090000,         // Typed text inserted at end of list
  URLBOX_STATIC         = 0x00100000,         // Unchangable text box
  URLBOX_NORMAL         = 0                   // Can type text but list is not changed
  };
...
*/
class FXAPI FXURLBox : public FXPacker {
  FXDECLARE(FXURLBox)
protected:
  FXLabel       *label;
  FXTextField   *field;
...
protected:
  FXURLBox(){}
private:
  FXURLBox(const FXURLBox&);
  FXURLBox &operator=(const FXURLBox&);
public:
  ...
  long onUpdButton(FXObject*,FXSelector,void*);
public:
  enum {
...
public:
  ...
  FXURLBox(FXComposite *p,FXint cols,FXObject* tgt=NULL,FXSelector sel=0,FXuint opts=COMBOBOX_NORMAL,FXint x=0,FXint y=0,FXint w=0,FXint h=0,FXint pl=DEFAULT_PAD,FXint pr=DEFAULT_PAD,FXint pt=DEFAULT_PAD,FXint pb=DEFAULT_PAD);
  ...
  void setDelimiters(const FXString &);
